...
const Events = () => {
  const EventCards = [
    ...
    },
    {
      ...
      date: "2025 . 10 . 10", // updated date following the same format
      time: "2:00 PM",
      imageUrl: EventImage1,
    },
    {
    ...
    <section className="flex flex-col xl:flex-row justify-between items-start gap-8 px-4 xl:px-0 py-10 bg-[#FDFDFD] font-manrope">
      ...
      >
        {EventCards.map((event) => (
          <div
            key={event.id}
            className="min-w-[300px]  shrink-0 relative border border-gray-200 transition-transform duration-300 ease-in-out hover:scale-105 "
            style={{ height: "420px" }}
          >
            <img
              src={event.imageUrl}
